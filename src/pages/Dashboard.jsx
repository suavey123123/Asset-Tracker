      ...
      <main style={{ flex:1, overflow:'auto', background:'var(--bg)', minWidth:0 }}>
        ...
        <ErrorBoundary>
          <div key={viewingAsset ? `asset-${viewingAsset.id}` : `tab-${tab}`} style={{ padding:'1rem 1rem' }}>
            {viewingAsset ? (
              <AssetDetail key={`asset-${viewingAsset.id}-${detailKey}`} assetId={viewingAsset.id} onBack={()=>{ setViewingAsset(null); setTab(viewingAssetFromTab); setEditModalAsset(null) }} onEdit={handleEdit} onRefetch={setViewingAsset} />
            ) : renderActivePage()}
          </div>
        </ErrorBoundary>
