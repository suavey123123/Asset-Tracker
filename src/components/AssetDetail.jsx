...
export default function AssetDetail({ assetId, onBack, onEdit }) {
  ...
  useEffect(() => {
    ...
    script.onload = () => {
      if (window.QRCode && qrRef.current) {
        new window.QRCode(qrRef.current, { text: url, width: size, height: size, colorDark: '#e8e8e8', colorLight: '#161616' })
      }
    }
  ...
  function printQR() {
    ...
    win.document.write(`
      ...
      <style>body{font-family:monospace;text-align:center;padding:20px}h2{font-size:14px}p{font-size:12px;color:#666}</style>
      </head><body>
      <h2>${asset.name}</h2>
      <p>${asset.asset_tag}</p>
      ${qrRef.current?.innerHTML || ''}
  ...
  return (
    <div className="fade-in">
      ...
      {/* Alerts */}
      {isOverdue && (
        <div style={{ background: 'var(--red-bg)', border: '1px solid var(--red)', borderRadius: 'var(--radius)', padding: '8px 14px', fontSize: 13, color: 'var(--red)', marginBottom: 10 }}>
          ⚠ This asset is overdue — expected return was {new Date(asset.expected_return).toLocaleDateString()}
        </div>
      )}
      {warrantyExpired && (
        <div style={{ background: 'var(--bg3)', border: '1px solid var(--border2)', borderRadius: 'var(--radius)', padding: '8px 14px', fontSize: 13, color: 'var(--text2)', marginBottom: 10 }}>
          Warranty expired {new Date(asset.warranty_expiry).toLocaleDateString()}
        </div>
      )}
      {warrantyExpiringSoon && (
        <div style={{ background: 'var(--amber-bg)', border: '1px solid var(--amber)', borderRadius: 'var(--radius)', padding: '8px 14px', fontSize: 13, color: 'var(--amber)', marginBottom: 10 }}>
      ...
      <div style={{ ...card, marginBottom: '1rem' }}>
        ...
        {maintenance.length === 0 ? (
          <div style={{ fontSize: 13, color: 'var(--text3)' }}>No maintenance records.</div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr>
                {['Type', 'Date', 'Performed by', 'Cost', 'Notes'].map(h => (
                  <th key={h} style={{ textAlign: 'left', padding: '6px 0', fontSize: 11, color: 'var(--text2)', fontWeight: 500, borderBottom: '1px solid var(--border)' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
