...
export default function QRLabels() {
  ...
  function printLabels() {
    ...
    const labelGrid = selectedAssets.map((a, i) => {
      const qrUrl = 'https://api.qrserver.com/v1/create-qr-code/?size=70x70&data=' + encodeURIComponent(appUrl + '/#asset=' + a.asset_tag)
      return '<div class="label"><img src="' + qrUrl + '" width="70" height="70" /><div class="info"><div class="tag">' + a.asset_tag + '</div><div class="model">' + (a.model || a.category || '').substring(0, 28) + '</div>' + (a.assigned_to ? '<div class="person">' + a.assigned_to.substring(0, 24) + '</div>' : '') + '</div></div>'
    }).join('')

    const html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>QR Labels</title><style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:Arial,sans-serif;background:white}.page{width:8.5in;padding:0.5in 0.1875in}.grid{display:grid;grid-template-columns:repeat(3,2.625in);gap:0 0.125in}.label{width:2.625in;height:1in;padding:4px 6px;display:flex;align-items:center;gap:6px;page-break-inside:avoid;overflow:hidden;border:0.5px solid #ddd}img{flex-shrink:0}.info{flex:1;min-width:0}.tag{font-family:monospace;font-size:8.5pt;font-weight:700;color:#000;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.model{font-size:7pt;color:#444;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-top:2px}.person{font-size:6.5pt;color:#666;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-top:1px}.print-btn{margin:12px;padding:8px 20px;font-size:14px;cursor:pointer;background:#d4ff4e;border:none;border-radius:4px;font-weight:600}@media print{.print-btn{display:none}.page{padding:0.5in 0.1875in}}</style></head><body><button class="print-btn" onclick="window.print()">Print ' + selectedAssets.length + ' labels</button><div class="page"><div class="grid">' + labelGrid + '</div></div></body></html>'
