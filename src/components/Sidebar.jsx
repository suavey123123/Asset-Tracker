    ...
    <aside style={s.sidebar}>
      ...
      <nav style={s.nav}>
        {NAV.filter(n=>!n.adminOnly||isAdmin).map(n=>(
          <button key={n.id} onClick={()=>onNav(n.id)} style={{ ...s.navItem, ...(active===n.id?s.navActive:{}) }}>
            <span style={s.navIcon}>{n.icon}</span>
            {n.label}
            {n.id==='home' && alerts>0 && (
              <span style={{ marginLeft:'auto', background:'var(--red)', color:'#fff', fontSize:10, fontWeight:600, padding:'1px 6px', borderRadius:100, fontFamily:'var(--mono)' }}>{alerts}</span>
            )}
          </button>
        ))}
      </nav>
      <div style={s.footer}>
        ...
        <div style={s.userRow}>
          ...
          <div style={{ flex:1, minWidth:0 }}>
            <div style={{ fontSize:12, fontWeight:500, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{profile?.full_name||profile?.email}</div>
