...
John Smith,john@company.com,Engineer,IT,555-1234,Floor 3 Desk 12,2024-01-15,
Jane Doe,jane@company.com,Manager,Operations,555-5678,Floor 2 Office 5,2023-06-01,`
  ...
  return (
    <Modal open={open} onClose={onClose} title="Import employees from CSV" width={600}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        {sites?.length > 0 && (
          <div>
            <label style={{ fontSize: 11, color: 'var(--text2)', display: 'block', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Assign all to site (optional)</label>
        ...
        {result && (
          <div style={{ background: 'var(--green-bg)', border: '1px solid var(--green)', borderRadius: 'var(--radius)', padding: '10px 14px', fontSize: 13, color: 'var(--green)' }}>
            ✓ Imported {result.count} employee{result.count !== 1 ? 's' : ''}
          </div>
        )}

        {preview.length > 0 && !result && (
          <div>
            <div style={{ fontSize: 11, color: 'var(--text2)', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Preview</div>
            <div style={{ border: '1px solid var(--border)', borderRadius: 'var(--radius)', overflow: 'hidden' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                <thead><tr style={{ borderBottom: '1px solid var(--border)' }}>
                  {['Name','Email','Title','Department','Hire date'].map(h => <th key={h} style={{ padding: '6px 10px', textAlign: 'left', color: 'var(--text2)', fontWeight: 500 }}>{h}</th>)}
                </tr></thead>
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', paddingTop: 8, borderTop: '1px solid var(--border)' }}>
          ...
          <Btn variant="primary" onClick={doImport} disabled={!csv.trim() || importing || errors.length > 0}>
            {importing ? 'Importing…' : `Import ${csv.trim() && !errors.length ? parseCSV(csv).rows.length + ' ' : ''}employees`}
          </Btn>
        </div>
      </div>
    </Modal>
